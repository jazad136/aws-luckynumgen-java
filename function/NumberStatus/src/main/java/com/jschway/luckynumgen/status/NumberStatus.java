...
import com.jschway.luckynumgen.ListBundle;
import com.jschway.luckynumgen.ListBundleMessage;
    ...
    public static APIGatewayProxyResponseEvent maxout(final APIGatewayProxyRequestEvent input, final Context context) {
        ...
        if(numberIn.isEmpty()) { 
            output = """
                { 
                "type": "exception"
                "message": "Expected an integer but it was not provided" 
                }""";
            return response
        ...
        for(String digit : digits) { 
            ...
            String s3Outcome = Setup.getFromS3(s3Client, numberInBucket,numberInKey);
